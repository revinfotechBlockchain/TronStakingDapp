...
var ContractAddress  = "TRVepNRCW4p7HHxb3Ln8nHFSCiXzR7UGrC";
...
cron.schedule('0 0 0 * * *', async () => {
    ...
    tronWeb.setDefaultBlock('latest');
    var newContract = await tronWeb.contract().at(ContractAddress);
    await newContract.getpurchaseableTokens().call().then(async output => {
        await newContract.getTotalTrx().call().then(async out1 => {
            await newContract.getPriceToken().call().then(async out2 => {
                var transformTokens = new TransformTokens();
                    ...
                    transformTokens.RYZAvailable = (parseInt(output)/1000000).toFixed(6);
...
module.exports = {
    ...
    getReferralAmount: async (req, res) => {
            ...
            await newContract && newContract.getReferralAmount().call().then(async output => {
                let response = {status:true, amount:output.toString()/1000000};
                res.send(response);
            }).catch(err => {
            ...
            referalManager.findOne(query).then(async data =>
                ...
                referalManager.findOne(query2).then(async data2 =>{
                    ...
                    if(!data || addr == req.body.address){
                        res.send({status : false , response:"Invalid Referal Code"});
                    }
                    else if(data.Used == true){
                        res.send({status : false, response: "Referal Link Already Used"});
                    }
                        ...
                        // var newContract = await tronWeb.contract().at(ContractAddress);
                        if(addr && !addr== ""){
                            // await newContract && newContract.withdrawReferral(addr).send().then(async output => {
                                referalManager.updateOne(query, {
                                    ...
                                    console.log(err)
                                    let response = {status:false, message:"Invalid or Expired Referal Code!!!"};
                                    console.log(response)
                                    res.send(response);
                                    }
                                  });
                        ...
                        res.send({status : false , response: "Invalid or Expired Referal Code or Code already used!!!"})
                    }
                //console.log(data);
              });
            });}
            catch(err){
                res.send({status : false , response: "Invalid or Expired Referal Code!!!"})
            }
        },

        deleteRecord: async (req, res)=>{
            referalManager.findOneAndRemove({ UsedAddress: req.query.address }, (err, doc) => {
